"""内容标签词典与打标：把文本/候选标签映射为「按题材」的规范 tag。

设计：
  - TAG_ENTRIES 为有序「关键词正则 → 规范 tag」表（更具体/常见的题材在前）。
  - pick_tags(texts)  ：在文本中匹配词典，产出规范 tag（用于本机简评打标）。
  - filter_tags(tags) ：对联网候选 tag 做归一化包含匹配，只保留命中词典的题材项。
  - TAG_LIMIT         ：每作品 tag 数上限（全局统一常量，admin/media/completion 共用）。
  - 两处均去重并按 limit（默认 TAG_LIMIT）截断。

注意：这里只提供「示例词条」，请按自己的片库题材替换 / 扩充 TAG_ENTRIES。
"""
import re
...
_COMPILED = [(re.compile(p, re.I), tag) for p, tag in TAG_ENTRIES]
...
def filter_tags(tags: list[str], limit: int = TAG_LIMIT) -> list[str]:
