...
| 文件 | 覆盖 |
|---|---|
| `test_parser.py` | 文件名三时代解析、标题归一、评分换算、海报查找 |
| `test_scan_completion.py` | 扫描幂等 / 定点范围、联网补全流程 |
| `test_metadata.py` | 元数据候选清洗与长度约束 |
| `test_tagdict.py` | 题材词典打标、`TAG_LIMIT` |
...
| `test_covers.py` / `test_cover_extra.py` | 本地封面匹配、指定目录补全、视频帧标记 |
| `test_online_covers.py` | 联网封面搜索 / 审定候选 |
| `test_frames.py` | ffmpeg 抽帧（无 ffmpeg 自动 skip） |
