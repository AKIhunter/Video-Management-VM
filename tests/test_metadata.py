"""联网补全候选：约束校验（简介≤500字、tag≤30个且≤20字、去重）与 sample 解析。"""
import pytest

from app.services.metadata_provider import (SampleResolver, LocalResolver, _pick_best,
                                   build_keyword, extract_baidu_synopsis,
                                   extract_sample_post, extract_sample_results,
...
class TestValidateCandidate:
    ...
    def test_tags_dedup_and_limit(self):
        c = validate_candidate({"tags": [" 纯爱 ", "纯爱", "NTR", "", "NTR"] + ["x" * 30] * 40})
...
class TestSampleParse:
    def test_extract_results(self):
        ...
        out = extract_sample_results(html_text)
    ...
    def test_extract_post_synopsis_and_tags(self):
        html_text = (
            ...
            '<a rel="tag">纯爱</a><a rel="tag">NTR</a>'
            '<p>这是一个很长的剧情简介，用来测试是否可以捕获正文段落并作为简介候选使用。</p>'
            '</div>'
        )
        cand = extract_sample_post(html_text)
...
class TestHelpers:
    def test_sample_url_uses_wp_search(self):
        url = SampleResolver()._search_url("测试作品")
        assert url.startswith("https://media-db.example.com/wp/?s=")
        assert SampleResolver().base in url
...
class TestCascade:
    ...
    def test_cascade_falls_back_to_baidu_then_sample(self, monkeypatch):
        import app.services.metadata_provider as mp
        state = {"calls": 0}
        ...
        cand = CascadeResolver().resolve(row)
        assert state["calls"] >= 2  # 百度失败后才走 sample
